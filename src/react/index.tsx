...
export const GameCard = ({ type, slug, locale = "en", layout, className, assetBaseUrl }: GameCardProps): JSX.Element => { const card = getCard(type, slug); if (!card || locale !== "en") throw new Error(`Unknown ${type} card '${slug}' for locale '${locale}'.`); const isRole = type === "actor-role"; const isSpecial = type === "actor-special"; const isModifier = type === "asset-modifier"; const isActor = isRole || isSpecial; const isOverlay = isActor || isModifier; const presentation = actorPresentation(card); const footerHeight = isSpecial && (presentation?.special?.length ?? 0) > 25 ? 60 : card.footer && card.footer.length > 100 ? 60 : undefined; const roleRules = presentation ? <ActorRules presentation={presentation} assetBaseUrl={assetBaseUrl} description={card.description} /> : card.body ?? card.description; const specialRules = presentation?.special ? <span><span className={styles.srOnly}>{card.description}</span><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={presentation.special} /></span> : card.body ?? card.description; return <LayeredCard assetBaseUrl={assetBaseUrl} className={className} layout={layout} transparent={isOverlay} showHeader={isActor || type !== "actor-base"} actorBase={type === "actor-base"} actorOverlay={isSpecial} imageUri={isRole ? undefined : card.artworkPath} noun={isSpecial || isModifier || type === "actor-base" ? "" : card.title} adjective={isSpecial || isModifier ? card.title : undefined} nounDeck={card.deck ?? type} nounCornerIcon={cornerIconByFamily[type]} nounEffect={isSpecial ? roleRules : isModifier ? undefined : roleRules} adjectiveEffect={isSpecial || isModifier ? specialRules : card.footer} footerHeight={footerHeight} titleColor={titleToneBySlug[slug]} />; };
...
export const ActorCard = ({ baseLayerSlug, tacticalRoleSlug, tacticalSpecialSlug, custom, ...props }: ActorCardProps): JSX.Element => {
  ...
  return <LayeredCard {...props} actorBase={Boolean(baseLayerSlug)} actorOverlay={Boolean(tacticalSpecialSlug)} noun={role?.title ?? ""} adjective={special?.title} nounEffect={props.nounEffect ?? (rolePresentation ? <ActorRules presentation={rolePresentation} bonusPresentation={specialPresentation} assetBaseUrl={props.assetBaseUrl} description={role?.description} /> : role?.body ?? role?.description)} adjectiveEffect={props.adjectiveEffect ?? (specialPresentation?.special ? <span><span className={styles.srOnly}>{special?.description}</span><ActorCardTextWithIcons assetBaseUrl={props.assetBaseUrl} text={specialPresentation.special} /></span> : special?.body ?? special?.description)} nounDeck="actor" adjectiveDeck={tacticalSpecialSlug ? "base mod" : undefined} nounCornerIcon="/types/actor.png" adjectiveCornerIcon={tacticalSpecialSlug ? "/types/actor.png" : undefined} imageUri={baseLayerSlug ? actorImageUri(baseLayerSlug) : undefined} imageOverlayUri={tacticalSpecialSlug ? actorImageUri(tacticalSpecialSlug) : undefined} />;
};
...
const ActorRules = ({ presentation, bonusPresentation, assetBaseUrl, description }: { presentation: ActorPresentation; bonusPresentation?: ActorPresentation; assetBaseUrl?: string; description?: string }): JSX.Element => <span className={styles.actorRules}><span className={styles.srOnly}>{description}</span>{presentation.toughness || bonusPresentation?.toughnessBonus ? <span className={styles.actorRuleRow}><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={presentation.toughness ?? ""} /><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={bonusPresentation?.toughnessBonus ?? ""} /></span> : null}{presentation.actions?.map((action, index) => <span key={index} className={styles.actorRuleRow}><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={action} /><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={bonusPresentation?.actionBonuses?.[index] ?? ""} /></span>)}</span>;
