...
const SvgText = ({ x, y, width, height, children, fontSize, minFontSize = fontSize, color = "#121b23", fontFamily = "MightyDecksKalam", weight = 700, region, alignEnd = false }: { x: number; y: number; width: number; height: number; children?: ReactNode; fontSize: number; minFontSize?: number; color?: string; fontFamily?: string; weight?: number; region?: "main" | "footer"; alignEnd?: boolean }): JSX.Element => {
...
export const LayeredCard = ({ className, style, assetBaseUrl = "/mighty-decks/assets", backgroundUri = "/backgrounds/paper-with-image-shadow.png", imageUri, imageOverlayUri, noun = "Card", nounDeck, nounCornerIcon, adjective, adjectiveDeck, adjectiveCornerIcon, nounEffect, adjectiveEffect, titleColor = "#121b23", layout = "full", transparent = false, showHeader = true, footerHeight = 38, actorBase = false, actorOverlay = false, actorLayout = false }: LayeredCardProps): JSX.Element => {
  ...
  const actorArt = actorBase || actorOverlay;
  const artX = compact ? 57 : actorArt ? 20 : 24; const artY = compact ? 30 : actorArt ? 42 : 44; const artWidth = compact ? 90 : actorArt ? 164 : 156; const artHeight = compact ? 90 : actorArt ? 118 : 98;
  return <CardStyleBoundary><article className={[styles.card, className].filter(Boolean).join(" ")} style={style} aria-label={asLabel(compact ? compactTitle : noun)} data-card-layer={transparent ? "overlay" : "base"}><svg viewBox="0 0 204 332" width="100%" height="100%" role="img" aria-label={asLabel(compact ? compactTitle : noun)}><defs><pattern id={paperId} width="1" height="1" patternUnits="objectBoundingBox"><image href={background} width="204" height="332" preserveAspectRatio="xMidYMid slice" /></pattern><clipPath id={artId}><rect x={artX} y={artY} width={artWidth} height={artHeight} rx={actorArt ? 0 : 6} /></clipPath></defs>{!transparent ? <><rect x="1" y="1" width="202" height="330" rx="11" fill={`url(#${paperId})`} />{!actorBase ? <rect x="1" y="1" width="202" height="330" rx="11" fill="none" stroke="#6d5435" strokeWidth="2" /> : null}</> : null}{!compact && !transparent && showHeader ? <><text x="168" y="25" textAnchor="end" fill="#856a4c" style={{ fontFamily: "MightyDecksShantell", fontSize: 9, fontWeight: 700 }}>{asLabel(nounDeck)}</text>{cornerIcon ? <image href={cornerIcon} x="173" y="11" width="18" height="18" preserveAspectRatio="xMidYMid meet" /> : null}</> : null}{actorBase && image && !compact ? <image href={image} x="29" y="48" width="20" height="20" preserveAspectRatio="xMidYMid meet" /> : null}{image || overlay ? <g clipPath={`url(#${artId})`}>{image ? <image href={image} x={artX} y={artY} width={artWidth} height={artHeight} preserveAspectRatio="xMidYMid meet" /> : null}{overlay ? <image href={overlay} x={actorOverlay && !compact ? 24 : artX} y={actorOverlay && !compact ? 44 : artY} width={actorOverlay && !compact ? 156 : artWidth} height={actorOverlay && !compact ? 98 : artHeight} preserveAspectRatio="xMidYMid meet" opacity={actorOverlay ? .85 : 1} /> : null}</g> : null}{!compact && !transparent && (adjectiveDeck || adjectiveCorner) ? <g transform="translate(185 90) rotate(90)">{adjectiveCorner ? <image href={adjectiveCorner} x="-58" y="-10" width="18" height="18" preserveAspectRatio="xMidYMid meet" /> : null}<text x="-36" y="3" fill="#856a4c" style={{ fontFamily: "MightyDecksShantell", fontSize: 9, fontWeight: 700 }}>{asLabel(adjectiveDeck)}</text></g> : null}{compact ? <SvgText x={12} y={137} width={180} height={142} fontSize={30} minFontSize={24} color={titleColor}>{compactTitle}</SvgText> : <><SvgText x={16} y={148} width={172} height={20} fontSize={16} minFontSize={11} color="#121b23">{adjective}</SvgText><SvgText x={16} y={167} width={172} height={24} fontSize={20} minFontSize={12} color={titleColor}>{noun}</SvgText><><SvgText x={16} y={194} width={172} height={mainHeight} fontSize={11} minFontSize={8} color="#23303d" fontFamily="MightyDecksShantell" weight={400} region="main" alignEnd={actorLayout}>{nounEffect}</SvgText><SvgText x={16} y={footerY} width={172} height={footerHeight} fontSize={11} minFontSize={8} color="#121b23" fontFamily="MightyDecksShantell" region="footer">{adjectiveEffect}</SvgText></></>}</svg></article></CardStyleBoundary>;
};
...
const cornerIconByFamily: Record<CardFamily, string> = { outcome: "/types/outcome.png", effect: "/types/effect.png", stunt: "/types/stunt.png", "actor-base": "/types/actor.png", "actor-role": "/types/actor.png", "actor-special": "/types/actor.png", "asset-base": "/types/asset.png", "asset-modifier": "/types/asset.png", counter: "/types/counter.png", location: "/types/map.png" };
export const GameCard = ({ type, slug, locale = "en", layout, className, assetBaseUrl }: GameCardProps): JSX.Element => { const card = getCard(type, slug); if (!card || locale !== "en") throw new Error(`Unknown ${type} card '${slug}' for locale '${locale}'.`); if (type === "location") return <IllustratedCard title={card.title} artworkPath={card.artworkPath ?? ""} kind="location" layout={layout} className={className} assetBaseUrl={assetBaseUrl} />; if (type === "actor-base" && card.deck === "medieval") return <IllustratedCard title={card.title} artworkPath={card.artworkPath ?? ""} kind="actor" layout={layout} className={className} assetBaseUrl={assetBaseUrl} />; const isRole = type === "actor-role"; const isSpecial = type === "actor-special"; const isModifier = type === "asset-modifier"; const isActor = isRole || isSpecial; const isOverlay = isActor || isModifier; const presentation = actorPresentation(card); const footerHeight = card.footer && card.footer.length > 100 ? 60 : undefined; const roleRules = presentation ? <ActorRules presentation={isSpecial ? {} : presentation} bonusPresentation={isSpecial ? presentation : undefined} assetBaseUrl={assetBaseUrl} description={card.description} /> : card.body ?? card.description; const specialRules = presentation?.special ? <span><span className={styles.srOnly} data-card-description>{card.description}</span><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={presentation.special} /></span> : card.body ?? card.description; return <LayeredCard assetBaseUrl={assetBaseUrl} className={className} layout={layout} transparent={isOverlay} showHeader={isActor || type !== "actor-base"} actorBase={type === "actor-base"} actorOverlay={isSpecial} actorLayout={isActor} imageUri={isRole || isSpecial ? undefined : card.artworkPath} imageOverlayUri={isSpecial ? card.artworkPath : undefined} noun={isSpecial || isModifier || type === "actor-base" ? "" : card.title} adjective={isSpecial || isModifier ? card.title : undefined} nounDeck={card.deck ?? type} nounCornerIcon={cornerIconByFamily[type]} nounEffect={isSpecial ? roleRules : isModifier ? undefined : roleRules} adjectiveEffect={isSpecial || isModifier ? specialRules : card.footer} footerHeight={footerHeight} titleColor={titleToneBySlug[slug]} />; };
...
const actorImageUri = (slug: string): string => getCard("actor-base", slug)?.artworkPath ?? `/actors/base/${slug.replaceAll("_", "-")}.png`;
export const ActorCard = ({ baseLayerSlug, tacticalRoleSlug, tacticalSpecialSlug, custom, ...props }: ActorCardProps): JSX.Element => {
  ...
  const base = baseLayerSlug ? getCard("actor-base", baseLayerSlug) : undefined;
  if (base?.deck === "medieval" && !tacticalRoleSlug && !tacticalSpecialSlug) return <IllustratedCard title={base.title} artworkPath={base.artworkPath ?? ""} kind="actor" layout={props.layout} className={props.className} style={props.style} assetBaseUrl={props.assetBaseUrl} />;
  ...
  const rolePresentation = actorPresentation(role); const specialPresentation = actorPresentation(special);
  return <LayeredCard {...props} actorLayout actorBase={Boolean(baseLayerSlug)} actorOverlay={Boolean(tacticalSpecialSlug)} noun={role?.title ?? ""} adjective={special?.title} nounEffect={props.nounEffect ?? (rolePresentation ? <ActorRules presentation={rolePresentation} bonusPresentation={specialPresentation} assetBaseUrl={props.assetBaseUrl} description={role?.description} /> : role?.body ?? role?.description)} adjectiveEffect={props.adjectiveEffect ?? (specialPresentation?.special ? <span><span className={styles.srOnly} data-card-description>{special?.description}</span><ActorCardTextWithIcons assetBaseUrl={props.assetBaseUrl} text={specialPresentation.special} /></span> : special?.body ?? special?.description)} nounDeck="actor" adjectiveDeck={tacticalSpecialSlug ? "base mod" : undefined} nounCornerIcon="/types/actor.png" adjectiveCornerIcon={tacticalSpecialSlug ? "/types/actor.png" : undefined} imageUri={baseLayerSlug ? actorImageUri(baseLayerSlug) : undefined} imageOverlayUri={tacticalSpecialSlug ? actorImageUri(tacticalSpecialSlug) : undefined} />;
};
...
const actorIconPaths: Record<string, string> = {
  injury: "/effects/injury.png", distress: "/effects/distress.png", burning: "/effects/burning.png", freezing: "/effects/freezing.png", stuck: "/effects/stuck.png", hindered: "/effects/hindered.png", complication: "/effects/complication.png", boost: "/effects/boost.png", direct: "/textIcons/direct.png", heal: "/textIcons/heal.png", melee: "/textIcons/melee.png", push: "/textIcons/push.png", range: "/textIcons/range.png", ranged: "/textIcons/ranged.png", replace: "/textIcons/replace.png", shield: "/textIcons/shield.png", speed: "/textIcons/speed.png", splash: "/textIcons/splash.png", tactics: "/textIcons/tactics.png", toughness: "/textIcons/toughness.png",
};
...
const actorPresentation = (card: ReturnType<typeof getCard> | undefined): ActorPresentation | undefined => {
  if (!card || (card.family !== "actor-role" && card.family !== "actor-special")) return undefined;
...
const ActorRules = ({ presentation, bonusPresentation, assetBaseUrl, description }: { presentation: ActorPresentation; bonusPresentation?: ActorPresentation; assetBaseUrl?: string; description?: string }): JSX.Element => {
  // Both layers reserve all three rows so a missing stat never moves another layer.
  const rows = [
    [presentation.toughness, bonusPresentation?.toughnessBonus],
    [presentation.actions?.[0], bonusPresentation?.actionBonuses?.[0]],
    [presentation.actions?.[1], bonusPresentation?.actionBonuses?.[1]],
  ];
  return <span className={styles.actorRules}><span className={styles.srOnly} data-card-description>{description}</span>{rows.map(([base, bonus], index) => <span key={index} className={styles.actorRuleRow}><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={base ?? ""} /><ActorCardTextWithIcons assetBaseUrl={assetBaseUrl} text={bonus ?? ""} /></span>)}</span>;
};
