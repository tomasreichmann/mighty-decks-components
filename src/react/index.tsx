...
export const GameCard = ({ type, slug, locale = "en", layout, className, assetBaseUrl }: GameCardProps): JSX.Element => { const card = getCard(type, slug); if (!card || locale !== "en") throw new Error(`Unknown ${type} card '${slug}' for locale '${locale}'.`); const isRole = type === "actor-role"; const isSpecial = type === "actor-special"; const isModifier = type === "asset-modifier"; const isOverlay = isRole || isSpecial || isModifier; const footerHeight = card.footer && card.footer.length > 100 ? 60 : undefined; return <LayeredCard assetBaseUrl={assetBaseUrl} className={className} layout={layout} transparent={isOverlay} showHeader={type !== "actor-base"} imageUri={isRole ? undefined : card.artworkPath} noun={isSpecial || isModifier || type === "actor-base" ? "" : card.title} adjective={isSpecial || isModifier ? card.title : undefined} nounDeck={card.deck ?? type} nounCornerIcon={cornerIconByFamily[type]} nounEffect={isSpecial || isModifier ? undefined : card.body ?? card.description} adjectiveEffect={isSpecial || isModifier ? card.body ?? card.description : card.footer} footerHeight={footerHeight} titleColor={titleToneBySlug[slug]} />; };
