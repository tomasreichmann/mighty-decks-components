...
const cornerIconByFamily = { outcome: "/types/outcome.png", effect: "/types/effect.png", stunt: "/types/stunt.png", "actor-base": "/types/actor.png", "actor-role": "/types/actor.png", "actor-special": "/types/actor.png", "asset-base": "/types/asset.png", "asset-modifier": "/types/asset.png", counter: "/types/counter.png", location: "/types/map.png" };
export const GameCard = ({ type, slug, locale = "en", layout, className, assetBaseUrl }) => { const card = getCard(type, slug); if (!card || locale !== "en")
    throw new Error(`Unknown ${type} card '${slug}' for locale '${locale}'.`); if (type === "location")
    return _jsx(IllustratedCard, { title: card.title, artworkPath: card.artworkPath ?? "", kind: "location", layout: layout, className: className, assetBaseUrl: assetBaseUrl }); if (type === "actor-base" && card.deck === "medieval")
    return _jsx(IllustratedCard, { title: card.title, artworkPath: card.artworkPath ?? "", kind: "actor", layout: layout, className: className, assetBaseUrl: assetBaseUrl }); const isRole = type === "actor-role"; const isSpecial = type === "actor-special"; const isModifier = type === "asset-modifier"; const isActor = isRole || isSpecial; const isOverlay = isActor || isModifier; const presentation = actorPresentation(card); const footerHeight = isSpecial ? 75 : card.footer && card.footer.length > 100 ? 60 : undefined; const roleRules = presentation ? _jsx(ActorRules, { presentation: presentation, assetBaseUrl: assetBaseUrl, description: card.description }) : card.body ?? card.description; const specialRules = presentation?.special ? _jsxs("span", { children: [_jsx("span", { className: styles.srOnly, "data-card-description": true, children: card.description }), _jsx(ActorCardTextWithIcons, { assetBaseUrl: assetBaseUrl, text: presentation.special })] }) : card.body ?? card.description; return _jsx(LayeredCard, { assetBaseUrl: assetBaseUrl, className: className, layout: layout, transparent: isOverlay, showHeader: isActor || type !== "actor-base", actorBase: type === "actor-base", actorOverlay: isSpecial, imageUri: isRole ? undefined : card.artworkPath, noun: isSpecial || isModifier || type === "actor-base" ? "" : card.title, adjective: isSpecial || isModifier ? card.title : undefined, nounDeck: card.deck ?? type, nounCornerIcon: cornerIconByFamily[type], nounEffect: isSpecial ? roleRules : isModifier ? undefined : roleRules, adjectiveEffect: isSpecial || isModifier ? specialRules : card.footer, footerHeight: footerHeight, titleColor: titleToneBySlug[slug] }); };
...
const actorImageUri = (slug) => getCard("actor-base", slug)?.artworkPath ?? `/actors/base/${slug.replaceAll("_", "-")}.png`;
export const ActorCard = ({ baseLayerSlug, tacticalRoleSlug, tacticalSpecialSlug, custom, ...props }) => {
    ...
    if (base?.deck === "medieval" && !tacticalRoleSlug && !tacticalSpecialSlug)
        return _jsx(IllustratedCard, { title: base.title, artworkPath: base.artworkPath ?? "", kind: "actor", layout: props.layout, className: props.className, style: props.style, assetBaseUrl: props.assetBaseUrl });
    ...
    return _jsx(LayeredCard, { ...props, actorBase: Boolean(baseLayerSlug), actorOverlay: Boolean(tacticalSpecialSlug), noun: role?.title ?? "", adjective: special?.title, nounEffect: props.nounEffect ?? (rolePresentation ? _jsx(ActorRules, { presentation: rolePresentation, bonusPresentation: specialPresentation, assetBaseUrl: props.assetBaseUrl, description: role?.description }) : role?.body ?? role?.description), adjectiveEffect: props.adjectiveEffect ?? (specialPresentation?.special ? _jsxs("span", { children: [_jsx("span", { className: styles.srOnly, "data-card-description": true, children: special?.description }), _jsx(ActorCardTextWithIcons, { assetBaseUrl: props.assetBaseUrl, text: specialPresentation.special })] }) : special?.body ?? special?.description), nounDeck: "actor", adjectiveDeck: tacticalSpecialSlug ? "base mod" : undefined, nounCornerIcon: "/types/actor.png", adjectiveCornerIcon: tacticalSpecialSlug ? "/types/actor.png" : undefined, imageUri: baseLayerSlug ? actorImageUri(baseLayerSlug) : undefined, imageOverlayUri: tacticalSpecialSlug ? actorImageUri(tacticalSpecialSlug) : undefined });
};
...
const ActorRules = ({ presentation, bonusPresentation, assetBaseUrl, description }) => _jsxs("span", { className: styles.actorRules, children: [_jsx("span", { className: styles.srOnly, "data-card-description": true, children: description }), presentation.toughness || bonusPresentation?.toughnessBonus ? _jsxs("span", { className: styles.actorRuleRow, children: [_jsx(ActorCardTextWithIcons, { assetBaseUrl: assetBaseUrl, text: presentation.toughness ?? "" }), _jsx(ActorCardTextWithIcons, { assetBaseUrl: assetBaseUrl, text: bonusPresentation?.toughnessBonus ?? "" })] }) : null, presentation.actions?.map((action, index) => _jsxs("span", { className: styles.actorRuleRow, children: [_jsx(ActorCardTextWithIcons, { assetBaseUrl: assetBaseUrl, text: action }), _jsx(ActorCardTextWithIcons, { assetBaseUrl: assetBaseUrl, text: bonusPresentation?.actionBonuses?.[index] ?? "" })] }, index))] });
