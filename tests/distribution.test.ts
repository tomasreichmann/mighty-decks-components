...
const runVerifier = (root: string, source?: string): Promise<{ code: number; output: string }> => new Promise((resolveRun, reject) => {
...
const runPrepare = (source: string, output: string, skipBuild = true): Promise<{ code: number; output: string }> => new Promise((resolveRun, reject) => {
...
const copyFixtureSources = async (source: string): Promise<void> => {
  await Promise.all([
...
test("prepares a script-free runtime with public assets and an inventory", async () => {
  ...
  try {
    ...
    const runtimePackage = JSON.parse(await readFile(join(output, "runtime", "package.json"), "utf8")) as { scripts?: unknown };
