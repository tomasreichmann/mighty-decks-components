...
const ruleIds = async (code, filePath) => (await eslint.lintText(code, { filePath })).map((result) => result.messages.map((message) => message.ruleId ?? "parse-error")).flat();
const expectRule = async (code, filePath, ruleId) => {
