...
test("the English rulebook teaches Asset and Actor card composition", async () => {
    const rulebook = await readFile(resolve(packageRoot, "docs/en/mighty-decks-rulebook.md"), "utf8");
...
test("the fast-session prompt directs the storyteller to the composition rules", async () => {
    const prompt = await readFile(resolve(packageRoot, "docs/en/mighty-decks-fast-session-storyteller-system-prompt.md"), "utf8");
