...
const git = (cwd: string, argumentsList: string[]) => exec("git", argumentsList, { cwd });
...
test("installs only safe local hooks and rejects a seeded lint violation", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => git(root, ["commit", "-m", "seeded lint violation"]),
      /no-explicit-any/,
    );
  } finally {
...
test("does not overwrite an existing hooks path", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => exec(process.execPath, ["scripts/install-hooks.mjs"], { cwd: root }),
      /core\.hooksPath is already/,
    );
