...
test("reports configured archive-host limit violations", () => {
  assert.throws(
    () => artifacts.assertWithinHostLimits({ compressedBytes: 11 }, { maxCompressedBytes: 10 }),
    /host limit/i,
  );
});
...
test("rejects PNG entries from the runtime package", () => {
  assert.throws(
    () => artifacts.assertRuntimeEntries(["package/generated/png/en/outcome/success/full/1024.png"]),
    /PNG/i,
  );
});
...
test("rejects a release manifest with mismatched versions or missing inventory", () => {
  assert.throws(
    () => artifacts.assertReleaseManifest({ packageVersion: "0.1.0", contentVersion: "one", pngs: [] }, { packageVersion: "0.1.1", contentVersion: "one", expectedPaths: [] }),
    /version/i,
  );
  assert.throws(
    () => artifacts.assertReleaseManifest({ packageVersion: "0.1.0", contentVersion: "one", pngs: [{ path: "mighty-decks/generated/png/en/a.png", checksum: "a" }] }, { packageVersion: "0.1.0", contentVersion: "one", expectedPaths: ["mighty-decks/generated/png/en/a.png", "mighty-decks/generated/png/en/b.png"] }),
    /missing/i,
  );
});
...
test("rejects checksum mismatches and archive paths outside the documented root", () => {
  assert.throws(
    () => artifacts.assertInventoryChecksums([{ path: "mighty-decks/generated/png/en/a.png", checksum: "wanted" }], new Map([["mighty-decks/generated/png/en/a.png", "actual"]])),
    /checksum/i,
  );
  assert.throws(
    () => artifacts.assertSafeArchivePaths(["../outside.png"]),
    /unsafe/i,
  );
});
