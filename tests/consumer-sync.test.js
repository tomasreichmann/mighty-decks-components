...
const run = async (command, args, cwd) => (await exec(command, args, { cwd })).stdout.trim();
const runSync = (cwd, args) => new Promise((resolveRun, reject) => {
...
const createDistribution = async (repository) => {
    const files = {
...
test("syncs selected runtime and core PNG files from a pinned Git commit", async () => {
    ...
    try {
        ...
        const lock = JSON.parse(await readFile(join(consumer, "mighty-decks.lock.json"), "utf8"));
